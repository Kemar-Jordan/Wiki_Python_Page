...
def message_about_scripts_not_on_PATH(scripts):
    ...
    last_line_fmt = ("Consider adding {} to PATH or, if you prefer "
...
def _install_wheel(
    ...
    def is_script_scheme_path(path):
        # type: (RecordPath) -> bool
        parts = path.split("/", 2)
        return (len(parts) > 2 and parts[0].endswith(".data") and
                parts[1] == "scripts")

    other_scheme_paths, script_scheme_paths = partition(is_script_scheme_path,
                                                        data_scheme_paths)
    ...
    distribution = pkg_resources_distribution_for_wheel(wheel_zip, name,
                                                        wheel_path)
